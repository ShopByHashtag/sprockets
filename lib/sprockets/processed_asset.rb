	...
	class ProcessedAsset < Asset
		def initialize(environment, logical_path, pathname)
			...
			begin
				...
				build_required_assets(environment, context) if context.present?

				begin
					build_dependency_paths(environment, context)
				rescue => e
					environment.logger.info "This path blew up and couldn't be found: #{logical_path}  (#{elapsed_time}ms)  (pid #{Process.pid})"
					Airbrake.notify(e)
				end
				...
				environment.logger.info "There was a problem processing: #{logical_path}  (#{elapsed_time}ms)  (pid #{Process.pid})"
				Airbrake.notify(e)
			end
		end
		...
		def build_dependency_paths(environment, context)
			...
			context._dependency_paths.each { |path|
				begin
					dep                   = DependencyFile.new(path, environment.stat(path).mtime, environment.file_digest(path).hexdigest)
					dependency_paths[dep] = true
				rescue => e
					environment.logger.info "There was a problem processing: #{path}!"
					Airbrake.notify(e)
				end
			}

			context._dependency_assets.each { |path|
				begin
					if path == self.pathname.to_s
